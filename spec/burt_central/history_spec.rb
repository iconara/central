...
describe BurtCentral::History do
  ...
  before do
    @date = Date.civil(2010, 3, 14)
    @event1 = BurtCentral::Event.new(:url => 'http://example.com/1', :title => 'Eventful',    :date => Date.civil(2010, 3, 10), :instigator => 'You',     :type => :tweet)
    @event2 = BurtCentral::Event.new(:url => 'http://example.com/2', :title => 'Foo',         :date => Date.civil(2010, 3, 15), :instigator => 'Me',      :type => :story)
    @event3 = BurtCentral::Event.new(:url => 'http://example.com/3', :title => 'Bar',         :date => Date.civil(2010, 3, 14), :instigator => 'Someone', :type => :error)
    @event4 = BurtCentral::Event.new(:url => 'http://example.com/4', :title => 'Hello World', :date => Date.civil(2010, 2, 28), :instigator => 'Him',     :type => :tweet)
  ...
  describe '#restore' do
    it 'asks the repository for events and creates Event objects' do
      @repository.should_receive(:find).with({}, {:sort => [:date, :descending]}).and_return([@event2.to_h, @event3.to_h, @event1.to_h, @event4.to_h])
